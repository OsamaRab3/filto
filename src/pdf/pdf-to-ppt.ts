...
export async function convertPdfToPptx(inputPdf: string, outputDocx: string): Promise<ConversionResult> {
    ...
    try {
        ...
        const result = await new Promise<AsposePdfConversionResult>((resolve) => {
            const conversion = AsposePdfModule.AsposePdfToPptX(inputPdf, outputDocx);
            resolve(conversion);
        });
        ...
        if (result.errorCode === 0) {
            return {
                success: true,
                message: `Successfully converted PDF to PPTX: ${outputDocx}`
            };
        } else {
            return {
                success: false,
