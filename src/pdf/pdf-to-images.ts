...
export async function convertPdfToImages(inputPdf: string, outputDocx: string): Promise<ConversionResult> {
    ...
    try {
        ...
        const result = await new Promise<AsposePdfConversionResult>((resolve) => {
            const conversion = AsposePdfModule.AsposePdfPagesToJpg(inputPdf, outputDocx.replace(/\.jpg$/, "{0:D2}.jpg"), 150);
            resolve(conversion);
        });
        ...
        if (result.errorCode === 0) {
            return {
                success: true,
                message: `Successfully converted PDF to Jpg: ${outputDocx}`
            };
        } else {
            return {
                success: false,
